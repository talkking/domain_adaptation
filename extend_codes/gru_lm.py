...
class GRULM(Model):
    ...
    def __init__(self, nhid, nproj, nvocab, nlayers, max_norm):
        ...
        self.rnn = nn.ModuleList(
            [nn.GRUCell(dec_in if i==0 else nhid, nhid) for i in range(nlayers)]
        )
    ...
    def forward_one_step(self, state, ys):
        ...
        eys = self.embed(ys)
        t2 = time.time()
        #print(f"embedding_time={t2-t1}")
        #sta = {}
        #sta['embedding_time'] = t2 - t1
        t1 = time.time()
        state = self.rnn_forward(eys, state)
        t2 = time.time()
        #print(f"rnn_time={t2-t1}")
        #sta['rnn_time'] = t2 - t1
